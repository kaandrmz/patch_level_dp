"""Plotting script for DeepLabV3+ performance experiments using SEML results."""
...
def get_deeplabv3plus_results():
    """Retrieve DeepLabV3+ experiment results from SEML database."""
    try:
        df = get_results(
            'deeplabv3plus_cityscapes_experiments',
            fields=[
                'result.epsilon',
                'result.test_miou',
        ...
        print(f"Found {len(df)} completed DeepLabV3+ experiments")
        return df

    except Exception as e:
        print(f"Could not fetch results for 'deeplabv3plus_cityscapes_experiments' collection: {e}")
        return pd.DataFrame()
...
def create_variance_plot(patch_stats, minibatch_stats):
    ...
    ax.set_title('DeepLabV3+ on Cityscapes', pad=20)
    ...
    filename = 'deeplabv3plus_cityscapes_performance_variance.pdf'
...
def main():
    """Main plotting function for DeepLabV3+ performance experiments."""
    print("=== DeepLabV3+ Cityscapes Performance Analysis ===")

    df = get_deeplabv3plus_results()

    if df.empty:
        print("No results found. Make sure DeepLabV3+ experiments have been run:")
        print("  seml deeplabv3plus_cityscapes_experiments status")
        return
