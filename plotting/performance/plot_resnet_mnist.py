"""Plotting script for ResNet-MNIST performance experiments using SEML results."""
...
def get_resnet_mnist_results():
    """Retrieve ResNet-MNIST experiment results from SEML database."""
    try:
        df = get_results(
            'resnet_mnist_main',
            fields=[
        ...
        print(f"Found {len(df)} completed ResNet-MNIST experiments")
        return df

    except Exception as e:
        print(f"Could not fetch results for 'resnet_mnist_main' collection: {e}")
        return pd.DataFrame()
...
def create_variance_plot(patch_stats, minibatch_stats):
    """Create the variance plot with error bars (original 2-line version)."""
    plt.rcParams.update({
        "text.usetex": True,
    ...
    ax.set_title('ResNet-18 on MNIST', pad=20)
    ...
    filename = 'resnet_mnist_performance_variance.pdf'
...
def create_per_seed_plots(df_patch, df_minibatch):
    """Create separate plots for each seed showing patch-level vs minibatch comparison (original 2-line version)."""
    plt.rcParams.update({
        "text.usetex": True,
        "text.latex.preamble": r"\usepackage[T1]{fontenc}\usepackage{lmodern}",
        'font.size': 30,
    ...
    for seed in seeds:
        ...
        filename = f'resnet_mnist_performance_seed_{seed}.pdf'
        plt.savefig(filename, dpi=300, bbox_inches='tight')
...
def create_variance_plot_3lines(patch_stats, minibatch_stats, gaussian_stats):
    """Create the variance plot with error bars including all three methods."""
    plt.rcParams.update({
        "text.usetex": True,
        "text.latex.preamble": r"\usepackage[T1]{fontenc}\usepackage{lmodern}",
        'font.size': 27,
    ...
    filename = 'resnet_mnist_performance_variance_3lines.pdf'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
...
def create_per_seed_plots_3lines(df_patch, df_minibatch, df_gaussian):
    """Create separate plots for each seed with all three methods."""
    plt.rcParams.update({
        "text.usetex": True,
        "text.latex.preamble": r"\usepackage[T1]{fontenc}\usepackage{lmodern}",
        'font.size': 30,
    ...
    for seed in seeds:
        ...
        filename = f'resnet_mnist_performance_seed_{seed}_3lines.pdf'
        plt.savefig(filename, dpi=300, bbox_inches='tight')
...
def main():
    """Main plotting function for ResNet-MNIST performance experiments."""
    print("=== ResNet-MNIST Performance Analysis ===")

    df = get_resnet_mnist_results()

    if df.empty:
        print("No results found. Make sure ResNet-MNIST experiments have been run:")
        print("  seml resnet_mnist_main status")
        return
