...
class ExtRandomCrop(object):
    ...
    def __call__(self, img, lbl):
        ...
        if self.pad_if_needed and h < self.size[0]:
            img = F.pad(img, (0, 0, 0, self.size[0] - h))
        if self.pad_if_needed and w < self.size[1]:
            img = F.pad(img, (0, 0, self.size[1] - w, 0))

        if self.pad_if_needed and h < self.size[0]:
            lbl = F.pad(lbl, (0, 0, 0, self.size[0] - h))
        if self.pad_if_needed and w < self.size[1]:
            lbl = F.pad(lbl, (0, 0, self.size[1] - w, 0))
        
        # The cropped image is resized using bilinear interpolation, but the label
        # must be resized using nearest neighbor to avoid corrupting class IDs.
        return F.resized_crop(img, i, j, h, w, self.size, Image.BILINEAR), \
